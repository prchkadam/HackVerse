    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 400 || response.status === 403) {
          this.onStatusChange("error");
          throw new Error(
            `Invalid API key (HTTP ${response.status}). Please check your Gemini API key in settings.`,
          );
        }
      ...
      if (
        ...
        err?.message?.includes("API validation failed")
      ) {
        throw err;
      }
      // Network error — could be offline, proceed anyway since REST is stateless
      console.warn(
        "[Gemini] Could not validate API key (network issue?):",
        err?.message,
      );
    }
  ...
  private async _request(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 400 || response.status === 403) {
          console.error(
            "[Gemini] ❌ API key appears invalid. Check your settings.",
          );
  ...
  async sendDetailedFrame(base64Jpeg: string): Promise<boolean> {
    ...
    try {
      const parts = [
        ...
        {
          text:
            "Describe everything you can see in this image in rich detail. " +
  ...
  async sendOcrFrame(base64Jpeg: string): Promise<boolean> {
    ...
    try {
      ...
      if (text) {
        ...
        this.onTranscript(text);
        const finalText =
          text.toLowerCase().includes("no text detected") ||
