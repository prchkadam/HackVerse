...
export class EmergencyService {
  ...
  async sendSOS(
    ...
    if (!contactNumber || contactNumber.trim().length < 3) {
      ...
      Alert.alert(
        "Setup Required",
        "Please open EchoSight Settings and set an Emergency Contact Number first!",
      );
    ...
    try {
      const accountSid =
        process.env.EXPO_PUBLIC_TWILIO_ACCOUNT_SID || "YOUR_TWILIO_ACCOUNT_SID";
      const authToken =
        process.env.EXPO_PUBLIC_TWILIO_AUTH_TOKEN || "YOUR_TWILIO_AUTH_TOKEN";
      const twilioNumber =
        process.env.EXPO_PUBLIC_TWILIO_PHONE_NUMBER || "YOUR_TWILIO_NUMBER";
      ...
      if (!response.ok) {
        ...
        console.warn("[Emergency] " + errorMessage);
