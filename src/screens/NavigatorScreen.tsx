...
export function NavigatorScreen() {
  ...
  const [specialMode, setSpecialMode] = React.useState<
    "none" | "detailed" | "ocr"
  >("none");
  ...
  const [currencyTotals, setCurrencyTotals] = React.useState<
    Record<string, number>
  >({});
  const [currentFrameTotals, setCurrentFrameTotals] = React.useState<
    Record<string, number>
  >({});
  const noteCountRef = useRef<number>(0);
  const currencyTotalsRef = useRef<Record<string, number>>({});
  const lastVisibleNotesRef = useRef<
  ...
  const device = React.useMemo(
    () => (isWeb ? ({ id: "mock" } as any) : deviceRaw),
    [isWeb, deviceRaw],
  );
    ...
    const requestAllPermissions = async () => {
      ...
      if (Platform.OS === "android") {
        try {
          ...
          console.warn(
            "[Permissions] Failed to request Android SOS permissions:",
            e,
          );
        }
      }
  ...
  useEffect(() => {
    if (!hasSpokenWelcome.current) {
      ...
      const welcomeText =
        "Welcome to EchoSight. Tap once to start or pause scanning. Double tap to stop. Triple tap to ask a question. Swipe up for a detailed description. Swipe down to read text. Long press for settings.";
  ...
  const handleBanknoteDetection = useCallback((rawText: string): string => {
    ...
    let summary: {
      total_bills: number;
      bills: Array<{ denomination: number; currency: string; count: number }>;
    };
    try {
    ...
    for (const bill of summary.bills) {
      ...
      if (
        currencyLower.startsWith("rupee") ||
        currencyLower === "rs" ||
        currencyLower === "inr"
      ) {
        currency = "Rupees";
      } else if (
        currencyLower.startsWith("dollar") ||
        currencyLower === "usd"
      ) {
        currency = "Dollars";
      ...
      frameTotals[currency] =
        (frameTotals[currency] || 0) + bill.denomination * count;
    }
    ...
    let restOfText = rawText
      .replace(notesMatchStr, "")
    ...
    geminiService.onTextResponse = (text) => {
      ...
      if (aiProvider === "gemini") {
        if (
          ...
          motionService.hasMovedSignificantly(latestSnapshotRef.current)
        ) {
          return;
        }
  ...
  const triggerEmergencySOS = useCallback(async () => {
    ...
    if (result.success) {
      ...
      ttsService.speak(
        "Failed to send message. Please seek help immediately.",
        "hazard",
      );
    }
  }, [emergencyContactNumber]);
  ...
  useEffect(() => {
    if (batterySaverEnabled) {
      ...
      batteryService.onLowBattery = (level) => {
        ...
        ttsService.speak(
          `Low battery at ${pct}%. Entering battery saver mode.`,
          "hazard",
        );
  ...
  const handleFeatherlessFrame = useCallback(
    async (base64Jpeg: string, snapshot: OrientationSnapshot) => {
  ...
  // ── Groq frame handler ─────────────────────────────────────────────
  const handleGroqFrame = useCallback(
    async (base64Jpeg: string, snapshot: OrientationSnapshot) => {
      const startTime = Date.now();
  ...
  const startScanning = useCallback(async () => {
    ...
    try {
      ...
      announce("Connected. Scanning your environment.");
    } catch (err: any) {
    ...
    if (!isScanning) {
      ...
      instructionInterval = setInterval(() => {
        ...
        if (!showFallBanner && !isSilenced) {
          ttsService.speak(
            "EchoSight is paused. Tap to start scanning. Double-tap to stop. Triple-tap to ask. Swipe up for detail. Swipe down to read text. Long press for settings.",
            "normal",
          );
        }
      }, 10000);
  ...
  const handleVoiceQuestion = useCallback(async () => {
    ...
    if (cachedAudio && cameraRef.current) {
      ...
      try {
        ...
        console.warn(
          "[Voice] Failed to capture frame for cached question:",
          err,
        );
        announce("Could not capture image. Please try again.");
      }
    ...
    tapTimer.current = setTimeout(() => {
      ...
      if (count >= 3) {
        ...
        if (isScanning && connectionStatus === "connected") {
          ...
          announce("Start scanning first, then triple-tap to ask a question.");
        }
      } else if (count === 2) {
      ...
      if (frame) {
        ...
        if (aiProvider === "gemini") {
          ...
          if (text) {
            ...
            processDescription(processedText);
            const finalText =
              processedText.toLowerCase().includes("no text detected") ||
              processedText.toLowerCase().includes("notes scanned")
                ? processedText
  ...
  return (
    <View style={styles.root}>
      ...
      {isWeb ? (
        ...
        >
          <Text style={{ color: "#40407a", fontSize: 18 }}>
            [ Web Preview: Camera Mock ]
          </Text>
        </View>
      ) : (
        <Camera
      ...
      <View style={styles.gestureTarget} {...panResponder.panHandlers}>
        <TouchableOpacity
          style={StyleSheet.absoluteFill}
          onPress={handleTap}
      ...
      <SafeAreaView style={styles.topBar} pointerEvents="box-none">
        <View style={styles.brand}>
          <Text style={styles.brandIcon} accessibilityElementsHidden>
            👁
          </Text>
      ...
      {noteCount > 0 && (
        ...
        >
          ...
          <Text style={styles.noteCounterCount}>
            Notes Scanned: {noteCount}
          </Text>
        ...
        <View
          style={[
            ...
            specialMode === "detailed"
              ? styles.modeBannerDetailed
              : styles.modeBannerOcr,
          ]}
          accessibilityRole="alert"
          accessibilityLiveRegion="assertive"
        >
      ...
      <SafeAreaView style={styles.bottomPanel} pointerEvents="box-none">
        <View style={styles.statusRow}>
          <Animated.View
            style={[
              styles.dot,
              { backgroundColor: statusCfg.dot, opacity: dotAnim },
            ]}
          />
          ...
          {isScanning && responseLatency > 0 && (
            <Text style={styles.latencyText}>{responseLatency}ms</Text>
          )}
        </View>
